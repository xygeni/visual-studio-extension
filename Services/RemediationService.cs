...
namespace vs2026_plugin.Services
{
    public class RemediationService
    {
        ...
        public async Task<FixData> LaunchRemediationPreviewAsync(string kind, string issueId, string fileUri, string xygeniInstallPath)
        {
            ...
            switch (kind)
            {
                ...
                case "ia_vulnerability":
                    return await PreviewDiffAiRemediationAsync(issueId, fileUri, xygeniInstallPath);
        ...
        private async Task<FixData> PreviewDiffAiRemediationAsync(string issueId, string fileUri, string xygeniInstallPath)
        {
            try
            {
                if (string.IsNullOrEmpty(issueId)) return new FixData { IssueTitle = "", TempFile = null, Explanation = null };
                ...
                // Call scanner (AI Security fix via 'util rectify --ai')
                var scanner = XygeniScannerService.GetInstance();
                await scanner.RunRectifyAiCommandAsync(tempFile, issue.Detector, issue.BeginLine.ToString(), xygeniInstallPath, _logger);
